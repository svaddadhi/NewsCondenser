...
router.get('/news', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(`https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&apiKey=${process.env.NEWSAPI_KEY}&language=en`);
    ...
    .map(article => ({
      ...
      title: article.title,
      description: article.description && article.description.length > maxLength 
